...
public class qConsoleTheme : INetworkSerializable, IConfigurable
{
    ...
    public Dictionary<string, qColor> customColors = new()
    {
    ...
    public qColor this[string s]
    {
        get
        {
            return s switch
            {
                qDebug.DEFAULT_TAG => defaultColor,
                qDebug.WARNING_TAG => warningColor,
                qDebug.ERROR_TAG => errorColor,
                _ => customColors.TryGetValue(s, out var cl) ? cl : defaultColor,
            };
        }
        set
        {

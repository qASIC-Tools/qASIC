...
public class qCommandArgument
{
    ...
    public bool TryGetEnum<T>(out T result) where T : struct, Enum
    {
        ...
        foreach (var item in values)
        {
            if (!item.ToString().Equals(arg, StringComparison.CurrentCultureIgnoreCase)) continue;
            result = item;
            return true;
        }
    ...
    public bool TryGetOptionIndex(string[] options, out int index)
    {
        ...
        {
            if (!arg.Equals(options[i], StringComparison.CurrentCultureIgnoreCase)) continue;
            index = i;
            return true;
        }
